...
class Date(val text : String, var frequency : Int = 0) extends NamedEntity
class Time(val text : String, var frequency : Int = 0) extends NamedEntity
class Money(val text : String, var frequency : Int = 0) extends NamedEntity
class Number(val text : String, var frequency : Int = 0) extends NamedEntity

object NamedEntityService {
  val classifier = {
    // how about different
    // val model = getClass.getResourceAsStream("/english.all.3class.distsim.crf.ser.gz")
    val model = getClass.getResourceAsStream("/english.muc.7class.distsim.crf.ser.gz")
      ...
      (entities, sentence) => {
        ...
        while (tokens.hasNext) {
          ...
          getAnnotationType(tokens.head) match {
            ...
            case "DATE"       => addNamedEntity(entities, new Date(getNamedEntityForAnnotation(tokens, "DATE")))
            case "TIME"       => addNamedEntity(entities, new Time(getNamedEntityForAnnotation(tokens, "TIME")))
            case "MONEY"       => addNamedEntity(entities, new Money(getNamedEntityForAnnotation(tokens, "MONEY")))
            case "NUMBER"       => addNamedEntity(entities, new Number(getNamedEntityForAnnotation(tokens, "NUMBER")))
            case _              => tokens.next
          }
